...
import { formatNumber, getDeviceType } from "@/lib/utils";
import { DonutChart } from "@tremor/react";
import { sql } from "drizzle-orm";
import { Fragment } from "react";

export default async function TopDevicesWidget() {
  const logs = await db
    .select({
  ...
  //   const chartData = logs.map(({ user_agent, count }) => ({
  //     name: user_agent || "Unknown",
  //     value: count,
  //     href: user_agent || undefined,
  //   }));
          ...
          <div className="justify-self-end">
            <div className="grid grid-cols-2 gap-y-1 gap-4">
              ...
              {logs.map(({ user_agent, visits }, index) => (
                <Fragment key={getDeviceType(user_agent)}>
                  <div className="flex items-center gap-2 text-sm leading-5 text-neutral-64 h-9 px-4 py-2 rounded-md z-10">
                    ...
                    />
                    <span>{getDeviceType(user_agent)}</span>
                  </div>

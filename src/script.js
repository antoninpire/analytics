...
function _getSessionId() {
  const start = localStorage.getItem(SESSION_START_STORAGE_KEY);
  if (start !== null) {
    const timeDiff = new Date().getTime() - new Date(start).getTime();
    if (timeDiff >= MAX_AGE) {
...
function _getSessionStart() {
  return localStorage.getItem(SESSION_START_STORAGE_KEY);
}
...
/*
    ...
    - Save session start in localStorage
    - Send request to server
       - If session already exists, simply update the duration only if duration is superior to previous duration + ADD PAGE HIT
       - Upsert visitor
       - Create session
       - Create page hit
*/
...
/*
  ...
  Same as on load, function will simply update the duration if session already exists

    - Get session start from localStorage
    - Remove session start from localStorage
    - Send request to server
       - Update session to get the correct duration
*/
